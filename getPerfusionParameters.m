function [ maxCorrCoef, af, dv, mtt ] = getPerfusionParameters( corrCoefs, afRange, dvRange, mttRange )
...
[iAF, iDV, iMTT] = ind2sub([length(afRange), length(dvRange), length(mttRange)], i);
af = afRange(iAF);
dv = dvRange(iDV);
mtt = mttRange(iMTT);
